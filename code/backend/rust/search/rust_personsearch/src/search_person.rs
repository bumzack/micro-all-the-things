pub mod filters_search_person {
    ...
    fn filter_entity_with_joined_or(req: SearchPersonList) -> impl Future<Output=Result<impl Reply + Sized, Infallible>> {
        //  println!("filter_entity for a list of person nconsts  {:?}", &req);

        let nconsts = req.nconsts.iter().map(|nconst| format!("nconst = {}", nconst)).collect::<Vec<String>>();
