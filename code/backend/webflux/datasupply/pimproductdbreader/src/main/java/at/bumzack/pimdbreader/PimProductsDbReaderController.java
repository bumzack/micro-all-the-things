...
import static at.bumzack.constt.UrlConsts.PimProductsUrl;
...
public class PimProductsDbReaderController {
    ...
    // curl http://localhost:8091/pim/products/0/5/12
    ...
    private Mono<String> sendToSolrWriter(final Product data) {
        final SolrWriter solrWriter = new SolrWriter("localhost", "8102", "/solr/product");
//        LOG.info("URL of Solr Writer {}", solrWriter.getRequestUrl());
    ...
    @RouterOperations({
            @RouterOperation(path = "/pim/products/{start}/{pagesize}/{limit}",
                    method = GET,
