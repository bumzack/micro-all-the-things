...
JSON=webflux_search_article.json

curl localhost:3030/api/backend/webflux_search_article | jq >service.json
curl localhost:3030/api/backend/openapiclient/webflux_search_article | jq > $JSON
...
OUTPUT_PATH=${REPO_PATH}/src/generated-clients/searcharticleservice
