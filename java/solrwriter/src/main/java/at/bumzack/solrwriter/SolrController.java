...
import at.bumzack.solrwriter.dto.B2BArticleChannelInfoData;
    ...
    public Mono<ServerResponse> addArticle(final ServerRequest request) throws WebClientResponseException {
        ...
        return article
                ...
                .flatMap(a -> webClient.post()
                        ...
                        .doOnNext(e -> LOG.info("solr response {}", e))
                        .doOnError(e -> LOG.error("error from Solr '{}'", e.getMessage()))
                        .flatMap(e -> ServerResponse.ok().body(BodyInserters.fromValue("SolrWriter says: all good"))));
    }

    @RouterOperations({
            @RouterOperation(path = "/solr/article",
                    method = POST,
                    operation = @Operation(operationId = "article",
                            requestBody = @RequestBody(content = @Content(schema = @Schema(implementation = B2BArticleChannelInfoData.class))),
