...
public class SearchProductsController {
        ...
        return request.bodyToMono(SearchRequestData.class)
                .flatMap(requestData -> {
                    if (nonNull(requestData.getCode())) {
                        return solrService.searchByCode(requestData.getCode(), requestData.getPageSize())
    ...
    private Mono<ServerResponse> extractProductList(final SolrResponse res) {
        final var result = new SearchResult();
        if (nonNull(res.getResponse())) {
            result.setProducts(res.getResponse().getDocs());
        }
        ...
        return request.bodyToMono(SearchRequestData.class)
                .flatMap(requestData -> {
                    if (nonNull(requestData.getText())) {
                        return solrService.searchTexts(requestData.getText(), requestData.getPageSize())
                                .flatMap(this::extractProductList);
                    }
                    return ServerResponse.status(HttpStatus.NOT_FOUND).body(BodyInserters.fromValue("not found"));
                });
