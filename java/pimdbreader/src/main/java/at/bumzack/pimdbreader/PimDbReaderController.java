...
public class PimDbReaderController {
    private static final Logger LOG = reactor.util.Loggers.getLogger(PimDbReaderController.class);

    // curl http://localhost:8080/solr/findArticles/0/5

    @NonNull
    public Mono<ServerResponse> findArticlesSolr(final ServerRequest request) throws WebClientResponseException {
    ...
    private Mono<ServerResponse> execRequest(final int start, final int pageSize, final int limit) {
        ...
        return fetchArticles(webClient, pageSize, start)
                ...
                .flatMap(articles -> {
                    ...
                    if (count > 0) {
                        LOG.error("{} error(s) occurred during the indexing ", count);
                        return ServerResponse.status(HttpStatus.INTERNAL_SERVER_ERROR)
                                .contentType(MediaType.TEXT_PLAIN)
                                .body(BodyInserters.fromValue(count + " error(s) occurred during the indexing"));
                    }
                    LOG.info("{} articles successfully indexed", articles.size());
                    return ServerResponse.ok().body(BodyInserters.fromValue("cnt: " + articles + " articles processed"));
                });
    }
    ...
    private static String getUrl(final int start, final int pageSize) {
        ...
        return "https://localhost:8002/rwaportalwebservices/public/readPaginated?" + s + "&" + ps;
    }
    ...
    private Mono<String> sendToSolrWriter(final B2BArticleChannelInfoData data) {
        final SolrWriter solrWriter = new SolrWriter("localhost", "8081", "/solr/article");
//        LOG.info("URL of Solr Writer {}", solrWriter.getRequestUrl());
    ...
    @RouterOperations({
            @RouterOperation(path = "/solr/findArticles/{start}/{pagesize}/{limit}",
                    method = GET,
                    operation = @Operation(operationId = "findArticles",
    ...
    public RouterFunction<ServerResponse> solrRoutes() {
        return route()
                .nest(RequestPredicates.path("/solr"),
                        builder -> builder
                                .GET("findArticles/{start}/{pagesize}/{limit}", this::findArticlesSolr)
                                .build())
                .build();
    }
}
